    ...
    public partial class frmEB : Form
    {
        ...
        public StringBuilder SETsbSql()
        {
            ...
            if (comboBox1.Text.ToString().Equals("業績"))
            {
                ...
                STR.AppendFormat(@" SELECT '1' AS 'SEQ','官網' AS 'KIND' ,CAST(SUM(TH013) AS INT) AS 'MONEY' FROM  [TK].dbo.COPTG,[TK].dbo.COPTH WITH (NOLOCK) WHERE  TG001=TH001 AND TG002=TH002 AND  TH001='A233' AND TH020='Y' AND TG005 IN ('102300','114000','116300') AND SUBSTRING(TH002,1,8)>='{0}' AND SUBSTRING(TH002,1,8)<='{1}' ", dateTimePicker2.Value.ToString("yyyyMMdd"), dateTimePicker3.Value.ToString("yyyyMMdd"));
                STR.AppendFormat(@" UNION ALL");
                STR.AppendFormat(@" SELECT '2' AS 'SEQ','現銷' AS 'KIND' ,CAST(SUM(TH013) AS INT) AS 'MONEY' FROM  [TK].dbo.COPTG,[TK].dbo.COPTH WITH (NOLOCK) WHERE  TG001=TH001 AND TG002=TH002 AND  TH001='A230' AND TH020='Y' AND TG005 IN ('102300','114000','116300') AND SUBSTRING(TH002,1,8)>='{0}' AND SUBSTRING(TH002,1,8)<='{1}' ", dateTimePicker2.Value.ToString("yyyyMMdd"), dateTimePicker3.Value.ToString("yyyyMMdd"));
                STR.AppendFormat(@" UNION ALL");
