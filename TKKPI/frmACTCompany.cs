        ...
        public StringBuilder SETsbSql()
        {
            ...
            else if(comboBox1.Text.ToString().Equals("實際與預算比較費用報表"))
            {
                ...
                else
                {
                    ...
                    STR.AppendFormat(@"  ,ISNULL((SELECT (SUM(CONVERT(INT,TB004)*TB007)) FROM [TK].dbo.ACTTA TA,[TK].dbo.ACTTB TB WHERE TA.TA001=TB.TB001 AND TA.TA002=TB.TB002 AND TB.TB005=TEMP.TB005 AND TB006 IN (SELECT MR001 FROM  [TK].dbo.ACTMR WITH(NOLOCK)  WHERE  SUBSTRING(TA003,1,4)='{0}' AND SUBSTRING(TA003,5,2)='{1}' AND MR002 LIKE (SELECT LTRIM(RTRIM(MR002))+'%' FROM [TK].dbo.ACTMR WITH(NOLOCK)  WHERE MR001 ='{2}')   ) ),0) AS '當月實際金額'", dateTimePicker1.Value.ToString("yyyy"), dateTimePicker1.Value.ToString("MM"), comboBox2.SelectedValue.ToString());
                    ...
                    STR.AppendFormat(@"  ,CAST(ISNULL((SELECT SUM(CONVERT(INT,TB004)*TB007) FROM [TK].dbo.ACTTA TA WITH (NOLOCK),[TK].dbo.ACTTB TB WITH (NOLOCK) WHERE TA.TA001=TB.TB001 AND TA.TA002=TB.TB002 AND TB.TB005=TEMP.TB005 AND  TB.TB006 IN (SELECT MR001 FROM  [TK].dbo.ACTMR WITH(NOLOCK)  WHERE  SUBSTRING(TA.TA003,1,4)='{0}' AND SUBSTRING(TA.TA003,5,2)<='{1}' AND MR002 LIKE (SELECT LTRIM(RTRIM(MR002))+'%' FROM [TK].dbo.ACTMR WITH(NOLOCK)  WHERE MR001 ='{2}')   ) ),0)  AS DECIMAL(18,2)) AS '實際費用累積'", dateTimePicker1.Value.ToString("yyyy"), dateTimePicker1.Value.ToString("MM"), comboBox2.SelectedValue.ToString());
