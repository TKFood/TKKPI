        ...
        public StringBuilder SETsbSql()
        {
            ...
            else if(comboBox1.Text.ToString().Equals("實際與預算比較費用報表"))
            {
                ...
                else
                {
                    ...
                    STR.AppendFormat(@"  SELECT  年度,月份,科目,科目名稱,部門代號,部門,當月實際金額,當月預算,ISNULL(CAST(ROUND(當月實際金額/NULLIF(當月預算,0),2)*100 AS DECIMAL(18,2) ),0) AS 當月費用達成率,實際費用累積,費用預算累積");
                    STR.AppendFormat(@"  ,ISNULL(CAST(ROUND(實際費用累積/NULLIF(費用預算累積,0),2)*100 AS DECIMAL(18,2) ),0) AS 年度累積達成率");
                    STR.AppendFormat(@"  ,年度預算總額");
                    STR.AppendFormat(@"  ,CAST(ISNULL(ROUND(實際費用累積/NULLIF(年度預算總額,0),2)*100 ,0)AS DECIMAL(18,2) ) AS 年度預算累積達成率");
                    STR.AppendFormat(@"  FROM ");
                    STR.AppendFormat(@"  ( ");
                    STR.AppendFormat(@"  SELECT '{0}' AS '年度','{1}' AS '月份',TEMP.TB005 AS '科目',MA003 AS '科目名稱','{2}' AS '部門代號',ME002 AS '部門'",dateTimePicker1.Value.ToString("yyyy"), dateTimePicker1.Value.ToString("MM"), comboBox2.SelectedValue.ToString());
                    STR.AppendFormat(@"  ,ISNULL((SELECT (SUM(CONVERT(INT,TB004)*TB007)) FROM [TK].dbo.ACTTA TA,[TK].dbo.ACTTB TB WHERE  TA.TA011='Y' AND TA.TA001=TB.TB001 AND TA.TA002=TB.TB002 AND TB.TB005=TEMP.TB005 AND TB006 IN (SELECT MR001 FROM  [TK].dbo.ACTMR WITH(NOLOCK)  WHERE  SUBSTRING(TA003,1,4)='{0}' AND SUBSTRING(TA003,5,2)='{1}' AND MR002 LIKE (SELECT LTRIM(RTRIM(MR002))+'%' FROM [TK].dbo.ACTMR WITH(NOLOCK)  WHERE MR001 ='{2}')   ) ),0) AS '當月實際金額'", dateTimePicker1.Value.ToString("yyyy"), dateTimePicker1.Value.ToString("MM"), comboBox2.SelectedValue.ToString());
                    STR.AppendFormat(@"  ,CAST(ISNULL((SELECT SUM(MK006) FROM [TK].dbo.ACTMK MK WITH (NOLOCK) WHERE MK.MK003=TEMP.TB005 AND MK.MK002='{0}' AND MK.MK005='{1}'AND MK.MK004 IN  (SELECT MR001 FROM  [TK].dbo.ACTMR WITH(NOLOCK)  WHERE MR002 LIKE (SELECT LTRIM(RTRIM(MR002))+'%' FROM [TK].dbo.ACTMR WITH(NOLOCK)  WHERE MR001 ='{2}') ) ),0)  AS DECIMAL(18,2)) AS '當月預算' ", dateTimePicker1.Value.ToString("yyyy"), dateTimePicker1.Value.ToString("MM"), comboBox2.SelectedValue.ToString());
                    STR.AppendFormat(@"  ,CAST(ISNULL((SELECT SUM(CONVERT(INT,TB004)*TB007) FROM [TK].dbo.ACTTA TA WITH (NOLOCK),[TK].dbo.ACTTB TB WITH (NOLOCK) WHERE  TA.TA011='Y' AND TA.TA001=TB.TB001 AND TA.TA002=TB.TB002 AND TB.TB005=TEMP.TB005 AND  TB.TB006 IN (SELECT MR001 FROM  [TK].dbo.ACTMR WITH(NOLOCK)  WHERE  SUBSTRING(TA.TA003,1,4)='{0}' AND SUBSTRING(TA.TA003,5,2)<='{1}' AND MR002 LIKE (SELECT LTRIM(RTRIM(MR002))+'%' FROM [TK].dbo.ACTMR WITH(NOLOCK)  WHERE MR001 ='{2}')   ) ),0)  AS DECIMAL(18,2)) AS '實際費用累積'", dateTimePicker1.Value.ToString("yyyy"), dateTimePicker1.Value.ToString("MM"), comboBox2.SelectedValue.ToString());
                    STR.AppendFormat(@"  ,CAST(ISNULL((SELECT SUM(MK006) FROM [TK].dbo.ACTMK MK WITH (NOLOCK) WHERE MK.MK003=TEMP.TB005 AND MK.MK002='{0}' AND MK.MK005<='{1}'AND MK.MK004 IN  (SELECT MR001 FROM  [TK].dbo.ACTMR WITH(NOLOCK)  WHERE MR002 LIKE (SELECT LTRIM(RTRIM(MR002))+'%' FROM [TK].dbo.ACTMR WITH(NOLOCK)  WHERE MR001 ='{2}') ) ),0)  AS DECIMAL(18,2)) AS '費用預算累積' ", dateTimePicker1.Value.ToString("yyyy"), dateTimePicker1.Value.ToString("MM"), comboBox2.SelectedValue.ToString());
                    STR.AppendFormat(@"  ,CAST(ISNULL((SELECT SUM(MK006) FROM [TK].dbo.ACTMK MK WITH (NOLOCK) WHERE MK.MK003=TEMP.TB005 AND MK.MK002='{0}' AND MK.MK004 IN  (SELECT MR001 FROM  [TK].dbo.ACTMR WITH(NOLOCK)  WHERE MR002 LIKE (SELECT LTRIM(RTRIM(MR002))+'%' FROM [TK].dbo.ACTMR WITH(NOLOCK)  WHERE MR001 ='{1}') ) ),0)  AS DECIMAL(18,2)) AS '年度預算總額' ", dateTimePicker1.Value.ToString("yyyy"), comboBox2.SelectedValue.ToString());
                    STR.AppendFormat(@"  FROM (");
                    STR.AppendFormat(@"  SELECT TB005");
                    STR.AppendFormat(@"  FROM [TK].dbo.ACTTA WITH(NOLOCK),[TK].dbo.ACTTB WITH(NOLOCK)");
                    STR.AppendFormat(@"  WHERE  TA001=TB001 AND TA002=TB002");
                    STR.AppendFormat(@"  AND (TB005 LIKE '5%' OR TB005 LIKE '6%' ) ");
                    STR.AppendFormat(@"  AND TB006 IN (SELECT MR001 FROM  [TK].dbo.ACTMR WITH(NOLOCK)  WHERE MR002 LIKE (SELECT LTRIM(RTRIM(MR002))+'%' FROM [TK].dbo.ACTMR WITH(NOLOCK)  WHERE MR001 ='{0}')   )", comboBox2.SelectedValue.ToString());
                    STR.AppendFormat(@"  AND SUBSTRING(TA003,1,4)='{0}' AND SUBSTRING(TA003,5,2)='{1}'", dateTimePicker1.Value.ToString("yyyy"), dateTimePicker1.Value.ToString("MM"));
                    STR.AppendFormat(@"  UNION ALL");
                    STR.AppendFormat(@"  SELECT MK003 ");
                    STR.AppendFormat(@"  FROM [TK].dbo.ACTMK");
                    STR.AppendFormat(@"  WHERE MK004 IN (SELECT MR001 FROM  [TK].dbo.ACTMR WITH(NOLOCK) ");
                    STR.AppendFormat(@"  WHERE (MK003 LIKE '5%' OR MK003 LIKE '6%' ) ");
                    STR.AppendFormat(@"  AND MR002 LIKE (SELECT LTRIM(RTRIM(MR002))+'%' FROM [TK].dbo.ACTMR WITH(NOLOCK)");
                    STR.AppendFormat(@"  WHERE MR001 ='{0}')) ", comboBox2.SelectedValue.ToString());
                    STR.AppendFormat(@"  AND SUBSTRING(MK001,1,4)='{0}' AND SUBSTRING(MK001,5,2)='{1}') AS TEMP", dateTimePicker1.Value.ToString("yyyy"), dateTimePicker1.Value.ToString("MM"));
                    STR.AppendFormat(@"  LEFT JOIN [TK].dbo.ACTMA WITH (NOLOCK)  ON TB005=MA001 ");
                    STR.AppendFormat(@"  LEFT JOIN [TK].dbo.CMSME WITH (NOLOCK) ON ME001='{0}' ", comboBox2.SelectedValue.ToString());
                    STR.AppendFormat(@"  GROUP BY TB005,MA003,ME002) AS TEMP2");
                    STR.AppendFormat(@"  UNION ALL  ");
                    STR.AppendFormat(@"  SELECT 'TOTAL','','','','','',SUM(當月實際金額),SUM(當月預算),ROUND(SUM(當月實際金額)/SUM(當月預算)*100,2),SUM(實際費用累積),SUM(費用預算累積),ROUND(SUM(實際費用累積)/SUM(費用預算累積)*100,2),SUM(年度預算總額),ROUND(SUM(實際費用累積)/SUM(年度預算總額)*100,2)  ");
                    STR.AppendFormat(@"  FROM ");
                    STR.AppendFormat(@"  ( SELECT  年度,月份,科目,科目名稱,部門代號,部門,當月實際金額,當月預算,ISNULL(CAST(ROUND(當月實際金額/NULLIF(當月預算,0),2)*100 AS DECIMAL(18,2) ),0) AS 當月費用達成率");
                    STR.AppendFormat(@"  ,實際費用累積,費用預算累積");
                    STR.AppendFormat(@"  ,ISNULL(CAST(ROUND(實際費用累積/NULLIF(費用預算累積,0),2)*100 AS DECIMAL(18,2) ),0) AS 年度累積達成率");
                    STR.AppendFormat(@"  ,年度預算總額");
                    STR.AppendFormat(@"  ,ISNULL(CAST(ROUND(實際費用累積/NULLIF(年度預算總額,0),2)*100 AS DECIMAL(18,2) ),0) AS 年度預算累積達成率");
                    STR.AppendFormat(@"  FROM ");
                    STR.AppendFormat(@"  ( ");
                    STR.AppendFormat(@"  SELECT '{0}' AS '年度','{1}' AS '月份',TEMP.TB005 AS '科目',MA003 AS '科目名稱','{2}' AS '部門代號',ME002 AS '部門'", dateTimePicker1.Value.ToString("yyyy"), dateTimePicker1.Value.ToString("MM"), comboBox2.SelectedValue.ToString());
                    STR.AppendFormat(@"  ,ISNULL((SELECT (SUM(CONVERT(INT,TB004)*TB007)) FROM [TK].dbo.ACTTA TA,[TK].dbo.ACTTB TB WHERE  TA.TA011='Y' AND TA.TA001=TB.TB001 AND TA.TA002=TB.TB002 AND TB.TB005=TEMP.TB005 AND  SUBSTRING(TA003,1,4)='{0}' AND SUBSTRING(TA003,5,2)='{1}' AND TB006 IN (SELECT MR001 FROM  [TK].dbo.ACTMR WITH(NOLOCK)  WHERE MR002 LIKE (SELECT LTRIM(RTRIM(MR002))+'%' FROM [TK].dbo.ACTMR WITH(NOLOCK)  WHERE MR001 ='{2}')   ) ),0) AS '當月實際金額'", dateTimePicker1.Value.ToString("yyyy"), dateTimePicker1.Value.ToString("MM"), comboBox2.SelectedValue.ToString());
                    STR.AppendFormat(@"  ,CAST(ISNULL((SELECT SUM(MK006) FROM [TK].dbo.ACTMK MK WITH (NOLOCK) WHERE MK.MK003=TEMP.TB005 AND MK.MK002='{0}' AND MK.MK005='{1}'AND MK.MK004 IN  (SELECT MR001 FROM  [TK].dbo.ACTMR WITH(NOLOCK)  WHERE MR002 LIKE (SELECT LTRIM(RTRIM(MR002))+'%' FROM [TK].dbo.ACTMR WITH(NOLOCK)  WHERE MR001 ='{2}') ) ),0)  AS DECIMAL(18,2)) AS '當月預算' ", dateTimePicker1.Value.ToString("yyyy"), dateTimePicker1.Value.ToString("MM"), comboBox2.SelectedValue.ToString());
                    STR.AppendFormat(@"  ,CAST(ISNULL((SELECT SUM(CONVERT(INT,TB004)*TB007) FROM [TK].dbo.ACTTA TA WITH (NOLOCK),[TK].dbo.ACTTB TB WITH (NOLOCK) WHERE  TA.TA011='Y' AND TA.TA001=TB.TB001 AND TA.TA002=TB.TB002 AND TB.TB005=TEMP.TB005 AND  SUBSTRING(TA003,1,4)='{0}' AND SUBSTRING(TA003,5,2)='{1}'  AND  TB.TB006 IN (SELECT MR001 FROM  [TK].dbo.ACTMR WITH(NOLOCK)  WHERE MR002 LIKE (SELECT LTRIM(RTRIM(MR002))+'%' FROM [TK].dbo.ACTMR WITH(NOLOCK)  WHERE MR001 ='{2}')   )),0)  AS DECIMAL(18,2)) AS '實際費用累積'", dateTimePicker1.Value.ToString("yyyy"), dateTimePicker1.Value.ToString("MM"), comboBox2.SelectedValue.ToString());
                    STR.AppendFormat(@"  ,CAST(ISNULL((SELECT SUM(MK006) FROM [TK].dbo.ACTMK MK WITH (NOLOCK) WHERE MK.MK003=TEMP.TB005 AND MK.MK002='{0}' AND MK.MK005<='{1}'AND MK.MK004 IN  (SELECT MR001 FROM  [TK].dbo.ACTMR WITH(NOLOCK)  WHERE MR002 LIKE (SELECT LTRIM(RTRIM(MR002))+'%' FROM [TK].dbo.ACTMR WITH(NOLOCK)  WHERE MR001 ='{2}') ) ),0)  AS DECIMAL(18,2)) AS '費用預算累積' ", dateTimePicker1.Value.ToString("yyyy"), dateTimePicker1.Value.ToString("MM"), comboBox2.SelectedValue.ToString());
                    STR.AppendFormat(@"  ,CAST(ISNULL((SELECT SUM(MK006) FROM [TK].dbo.ACTMK MK WITH (NOLOCK) WHERE MK.MK003=TEMP.TB005 AND MK.MK002='{0}' AND MK.MK004 IN  (SELECT MR001 FROM  [TK].dbo.ACTMR WITH(NOLOCK)  WHERE MR002 LIKE (SELECT LTRIM(RTRIM(MR002))+'%' FROM [TK].dbo.ACTMR WITH(NOLOCK)  WHERE MR001 ='{1}') ) ),0)  AS DECIMAL(18,2)) AS '年度預算總額' ", dateTimePicker1.Value.ToString("yyyy"), comboBox2.SelectedValue.ToString());
                    STR.AppendFormat(@"  FROM (");
                    STR.AppendFormat(@"  SELECT TB005");
                    ...
                    STR.AppendFormat(@"  LEFT JOIN [TK].dbo.ACTMA WITH (NOLOCK)  ON TB005=MA001 ");
                    STR.AppendFormat(@"  LEFT JOIN [TK].dbo.CMSME WITH (NOLOCK) ON ME001='{0}' ", comboBox2.SelectedValue.ToString());
                    STR.AppendFormat(@"  GROUP BY TB005,MA003,ME002) AS TEMP2 ) AS TEMP3");
                    STR.AppendFormat(@"  ");
                    STR.AppendFormat(@"  ");
                    STR.AppendFormat(@"  ");
                }
